...
export async function getCommunityPageData(preview) {
  const data = await getClient(preview).fetch(getCommunityPageDataQuery)

  return data
}

export async function getFAQSectionData(preview) {
  const data = await getClient(preview).fetch(getFAQSectionDataQuery)
